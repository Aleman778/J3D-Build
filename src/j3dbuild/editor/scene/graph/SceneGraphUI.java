...
import j3dbuild.editor.ui.AMenu;
...
public class SceneGraphUI extends ATree {
    ...
    public SceneGraphUI(SceneEditor scene) {
        this.scene = scene;
...
//                    }
    ...
    private void setPopupMenu() {
        ...
        itemRename.addActionListener((ActionEvent e) -> {
            SceneGraphNode node = scene.selection.getSingle();
            if (node != null) {
                startEditingAtPath(ATree.getPath(node.getPath()[node.getPath().length - 1]));
            }
        });
